...
export class CashReqListComponent implements OnInit {
  ...
  getCashReqsByStatus() {
    this.isLoading = true;
    this._cashRequisitionService.getCashRequsitionByStatus(this.status).subscribe(response => {
      this.cashReqDataSource.data = response;
      console.log(this.cashReqDataSource.data);
      this.isLoading = false
    }, error => {
  ...
  createReq() {
    this._router.navigate(['axis/finance-manager/requisitions/cash/create']);
  }

  viewReqVehilce(id: string) {
    this._router.navigateByUrl(`axis/finance-manager/requisitions/cash/${id}`)
  }
