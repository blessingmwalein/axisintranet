...
@Component({
  selector: 'app-update-used-funds',
  templateUrl: './update-used-funds.component.html',
  styleUrls: ['./update-used-funds.component.scss']
})
export class UpdateUsedFundsComponent implements OnInit {
  ...
  constructor(
    ...
    private _cardRequsitionService: CardRequisitionService,
  ...
  ngOnInit(): void {
    this.updateCashReleasedForm = this._formBuilder.group({
      actualUsedFunds: [0, [Validators.required]],
  ...
  save(): void {
    ...
    this._cardRequsitionService.updateUsedFunds(this.updateCashReleasedForm.value).subscribe((response) => {
