...
export class CardReqListComponent implements OnInit {
  ...
  getCardReqs() {
    this.isLoading = true;
    this._cardRequisitionService.getFilteredCardRequisitions(this.status == "employee" ? false :true).subscribe(response => {
  ...
  createReq() {
    this._router.navigate(['axis/manager/requisitions/card/create']);
  }

  viewReqVehilce(id: string) {
    this._router.navigateByUrl(`axis/manager/requisitions/card/${id}`)
  }
